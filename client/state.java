...
import client.graphics.buffer;
import client.graphics.draw;
import client.graphics.frame;
import client.graphics.matrix;
import client.graphics.occluder;
import client.graphics.opengl;
...
import land.structures.barrel;
import land.structures.complex;
import land.structures.crunch;
import land.structures.extra_crunch;
import land.structures.grassland;
import land.map;
import land.linedef;
import land.triangle;
import land.sector;
import land.structures.big_box;
import land.structures.box;
import land.structures.house;
import land.structures.plain_tile;
import land.structures.sweep;
import land.things.baron;
import land.things.base.decal;
import land.things.base.item;
import land.things.base.missile;
import land.things.base.particle;
import land.things.items.medkit;
...
import land.things.tree;
import land.things.thing;
import land.things.you;
...
public class state
{
    ...
    public state(client client)
    {
        ...
        gbuffer = new frame(1, 1, new int[]{GL_RGB, GL_RG16F}, new int[]{GL_RGB, GL_RG}, new int[]{GL_UNSIGNED_INT, GL_FLOAT}, false, true);
        ...
        fstor_2_pong = new frame(1, 1, new int[]{GL_RGB16F}, new int[]{GL_RGB}, new int[]{GL_FLOAT}, true, false);
        fstor_4_pong = new frame(1, 1, new int[]{GL_RGB16F}, new int[]{GL_RGB}, new int[]{GL_FLOAT}, true, false);
        fstor_8_pong = new frame(1, 1, new int[]{GL_RGB16F}, new int[]{GL_RGB}, new int[]{GL_FLOAT}, true, false);
        fshadow = new frame(1, 1, new int[]{GL_RED}, new int[]{GL_RED}, new int[]{GL_UNSIGNED_BYTE}, false, true);
        fstor_5_11 = new frame(1, 1, new int[]{GL_RGB16F}, new int[]{GL_RGB}, new int[]{GL_FLOAT}, false, false);
        ...
        for (int i = 0; i < 20; i++)
            add_light(new light(1.0f + seed.nextFloat() * 40, 1.0f + seed.nextFloat() * 40, 0.1f + seed.nextFloat() * 4.8f, seed.nextFloat() + 0.5f, seed.nextFloat(), seed.nextFloat(), 1.0f + seed.nextFloat() * 20.0f));
    ...
    private void geometry_pass()
    {
        ...
        // opengl.perspective(perspective, view_matrix, -you.x, -you.eye, -you.y, sin_x, cos_x, sin_y, cos_y);
        
        {
            ...
            for (int i = 0; i < 16; i++) opengl.model_view_matrix[i] = view_matrix[i];
    ...
    public void draw()
    {
        ...
        for (int i = 0; i < lights.size(); i++)
        {
            ...
            {
                if (you.x + you.radius >= li.x - li.radius && you.x - you.radius <= li.x + li.radius && you.y + you.radius >= li.z - li.radius && you.y - you.radius <= li.z + li.radius)
                {
                    glDepthFunc(GL_GEQUAL);
                    glCullFace(GL_FRONT);
                }
        ...
        else
        {
            ...
            /*opengl.framebuffer(0);
            opengl.program(p_bloom);
            opengl.view(0, 0, client.width, client.height);
            opengl.orthographic(orthographic, 0.0f, 0.0f);
            opengl.mvp();
            opengl.texture0(fstor.textures[0]);
            //opengl.texture1(fstor_2.textures[0]);
            opengl.texture1(fshadow.textures[0]);
            //opengl.texture2(fstor_4.textures[0]);
            opengl.texture3(fstor_8.textures[0]);
            opengl.draw_elements(bcanvas);*/
        ...
        draw.image(bgui[bid], (client.width - w) / 2 + you.hand_x * you_scale, client.height - h + you.hand_y * you_scale, w, h, 0, 0, 1, 1);
        bgui[bid].end();

...
public abstract class opengl
{
    ...
    private static FloatBuffer matrix_buffer = BufferUtil.newFloatBuffer(16);
    ...
    public static void vao(buffer b, int pos, int col, int tex, int norm)
    {
        ...
        glBufferStorage(GL_ARRAY_BUFFER, b.vertex_buffer.limit(), GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT);
        b.vertex_buffer = glMapBufferRange(GL_ARRAY_BUFFER, 0, b.vertex_buffer.limit(), GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT, null).order(ByteOrder.nativeOrder());
        
        b.ebo = glGenBuffers();
        
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, b.ebo);
        glBufferStorage(GL_ELEMENT_ARRAY_BUFFER, b.index_buffer.limit(), GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT);
        b.index_buffer = glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0, b.index_buffer.limit(), GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT, null).order(ByteOrder.nativeOrder());
        ...
        if (norm > 0)
        {
            ...
            // tangent
            glVertexAttribPointer(index, norm, GL_FLOAT, false, stride * Float.BYTES, (pos + col + tex + norm) * Float.BYTES);
            glEnableVertexAttribArray(index);
            index++;
            
            // bitangent
            glVertexAttribPointer(index, norm, GL_FLOAT, false, stride * Float.BYTES, (pos + col + tex + norm + norm) * Float.BYTES);
            glEnableVertexAttribArray(index);
            index++;
        }
    ...
    public static void update_framebuffer(frame f)
    {
        ...
        for (int i = 0; i < f.textures.length; i++)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, f.texture_internal_format[i], f.width, f.height, 0, f.texture_format[i], f.texture_type[i], (ByteBuffer)null);
        }
        
        if (f.depth)
        {
            glBindTexture(GL_TEXTURE_2D, f.depth_texture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, f.width, f.height, 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, (ByteBuffer)null);
        }
    }
    ...
    private static int shader(String file, int type)
    {
        ...
        {
            System.err.println("shader error: " + glGetShaderInfoLog(shader, 512));
            System.exit(0);
        }
    ...
    /*public static void perspective(float[] perspective, float[] view_matrix, float x, float y, float z, float sin_x, float cos_x, float sin_y, float cos_y)
    {
        matrix.identity(view_matrix);
        
        matrix.rotate_x(view_matrix, sin_x, cos_x);
        matrix.rotate_y(view_matrix, sin_y, cos_y);
        
        for (int i = 0; i < 16; i++) model_view_matrix[i] = view_matrix[i];
        
        matrix.translate(model_view_matrix, x, y, z);
        matrix.multiply(model_view_projection_matrix, perspective, model_view_matrix);
    }*/

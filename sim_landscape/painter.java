...
public class painter extends JPanel
{
    ...
    public painter()
    {
        ...
        tectonic_elevation.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_plate_elevation; repaint();}});
        tectonic_drift.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_plate_drift; repaint();}});
        tectonic_forces.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_plate_forces; repaint();}});
        ocean_land_elevation.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_ocean_land_elevation; repaint();}});
        pressure.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_pressure; repaint();}});
        default_winds.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_default_winds; repaint();}});
        winds.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_winds; repaint();}});
        ocean_currents.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_ocean_currents; repaint();}});
        temperature.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_temperature; repaint();}});
        precipitation.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_precipitation; repaint();}});
        biome.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e){view = view_biome; repaint();}});
        ...
        for (int y = 0; y < p.height; y++)
        {
            for (int x = 0; x < p.width; x++)
            {
                ...
                if (p.precipitation_created)
                {
                    switch (view)
                    {
                        case view_plate_elevation: tectonic_elevation(g, p, hex); break;
                        case view_plate_drift: tectonic_drift(g, p, hex); break;
                        case view_plate_forces: tectonic_forces(g, hex);break;
                        case view_ocean_land_elevation: ocean_land_elevation(g, p, hex); break;
                        case view_pressure: pressure(g, hex); break;
                        case view_default_winds: default_winds(g, hex, px, py); break;
                        case view_winds: winds(g, hex, px, py); break;
                        case view_ocean_currents: ocean_currents(g, hex); break;
                        case view_temperature: temperature(g, hex); break;
                        case view_precipitation: precipitation(g, hex); break;
                        case view_biome: biome(g, hex); break;
                    }
                }
                else if (p.biome_created) biome(g, hex);
                else if (p.temperature_created) temperature(g, hex);
                else if (p.currents_created) ocean_currents(g, hex);
                else if (p.winds_created) winds(g, hex, px, py);
                else if (p.pressure_created) pressure(g, hex);
                else if (p.ocean_land_created) ocean_land_elevation(g, p, hex);
                else if (p.tectonics_created) tectonic_elevation(g, p, hex);
            }
        }
    }
}

...
public abstract class triangulation
{
    ...
    private static boolean contains(vector[] poly, vector p)
    {
        ...
        {
            if ((poly[i].y > p.y != poly[j].y > p.y) && (p.x < (poly[j].x - poly[i].x) * (double)(p.y - poly[i].y) / (double)(poly[j].y - poly[i].y) + poly[i].x))
                odd = !odd;
    ...
    {
        if (interior_angle(a, b, c) > Math.PI) return false;
        ...
        for (int i = 0; i < polygon.size(); i++)
        {
            ...
            if (contains(triangle, p)) return false;
        }
    ...
    private static boolean valid(ArrayList<polygon_vertex> polygon, vector a, vector b)
    {
        for (int i = 0; i < polygon.size(); i++)
        {
            ...
            if (a != c && a != d && b != c && b != d && intersect(a, b, c, d)) return false;
        }
    ...
    private static ArrayList<polygon_vertex> populate(sector sector, boolean floor)
    {
        ...
        for (sector inner : sector.inside)
        {
            if (floor)
            {
                if (inner.c_floor == false) continue;
            }
            else
            {
                if (inner.c_ceil == false) continue;
            }
        ...
        for (int i = 0; i < points.size(); i++) points.get(i).index = i;
        ...
        for (int i = 0; i < vecs.length; i++)
        {
            ...
            if (original == null)
            {
                ...
                for (j = 0; j < points.size(); j++)
                {
                    ...
                    if (v.y < o.point.y || (v.y == o.point.y && v.x > o.point.x)) continue;
    ...
    private static void populate_links(ArrayList<polygon_vertex> points, sector sector, boolean clockwise)
    {
        ...
        for (int i = 0; i < vecs.length; i++)
        {
            ...
            if (clockwise)
            {
                if (i == 0) last = find(points, vecs[vecs.length - 1]);
                else last = find(points, vecs[i - 1]);
                
                if (i == vecs.length - 1) next = find(points, vecs[0]);
                else next = find(points, vecs[i + 1]);
            }
            else
            {
                if (i == 0) next = find(points, vecs[vecs.length - 1]);
                else next = find(points, vecs[i - 1]);
                
                if (i == vecs.length - 1) last = find(points, vecs[0]);
                else last = find(points, vecs[i + 1]);
            }
    ...
    private static void clean_population(ArrayList<polygon_vertex> points)
    {
        ...
        for (int i = 0; i < points.size(); i++) remaining.add(points.get(i));
        ...
        while (remaining.isEmpty() == false)
        {
            ...
            do
            {
                ...
                while (current.last.size() != 1) current.last.remove(1);
            ...
            while (todo.isEmpty() == false)
            {
                ...
                for (polygon_vertex pv : temp) todo.add(pv);
    ...
    private static ArrayList<polygon_vertex> classify(ArrayList<polygon_vertex> points)
    {
        ...
        for (int i = 0; i < points.size(); i++)
        {
            ...
            if (both_above && reflex)
            {
                ...
                {
                    start.add(pos);
                    if (console) System.out.println("start " + pos.point.x + ", " + pos.point.y);
                }
            }
            else if (both_above && reflex == false)
            {
                ...
                {
                    split.add(pos);
                    if (console) System.out.println("split " + pos.point.x + ", " + pos.point.y);
                }
            }
            ...
            else if (both_below && reflex == false)
            {
                ...
                {
                    merge.add(pos);
                    if (console) System.out.println("merge " + pos.point.x + ", " + pos.point.y);
                }
            }
        ...
        for (int i = 0; i < merge.size(); i++)
        {
            ...
            for (j = p.index + 1; j < points.size(); j++)
            {
                ...
                if (valid(points, p.point, diagonal.point)) break;
            }
            ...
            if (console) System.out.println("-- adding merge diagonal from (" + p.point.x + ", " + p.point.y + ") to (" + d.point.x + ", " + d.point.y + ")");
        }
        ...
        for (int i = 0; i < split.size(); i++)
        {
            ...
            if (d.merge)
            {
                if (console) System.out.println("-- split using same diagonal from merge (" + p.point.x + ", " + p.point.y + ") to (" + d.point.x + ", " + d.point.y + ")");
                continue;
            }
            ...
            if (console) System.out.println("-- adding split diagonal from (" + p.point.x + ", " + p.point.y + ") to (" + d.point.x + ", " + d.point.y + ")");
        }
        
        return start;
    }
    
    private static void clip(sector sector, boolean floor, ArrayList<triangle> triangles, ArrayList<vector> monotone, float scale)
    {
        int i = 0;
        ...
        while (monotone.size() > 3)
        {
            ...
            if (i_last == -1) i_last += monotone.size();
            if (i_next == monotone.size()) i_next -= monotone.size();
            ...
            if (i == monotone.size()) i = 0;
        }
        
        if (floor)
            triangles.add(new triangle(sector.floor, sector.t_floor, monotone.get(0), monotone.get(1), monotone.get(2), floor, scale));
        else
            triangles.add(new triangle(sector.ceil, sector.t_ceil, monotone.get(2), monotone.get(1), monotone.get(0), floor, scale));
    }
    ...
    private static void build(sector sector, boolean floor, ArrayList<triangle> triangles, float scale)
    {
        if (floor)
        {
            if (sector.t_floor == -1) return;
        }
        else
        {
            if (sector.t_ceil == -1) return;
        }
        ...
        if (console)
        {
            ...
            for (polygon_vertex pv : points) pv.print();
            System.out.println();
        }
        ...
        if (console) System.out.println(monotone_begin.size() + " monotone polygons");
            ...
            for (int i = 0; i < monotone_begin.size(); i++)
            {
                ...
                do
                {
                    ...
                    for (int j = 0; j < pos.last.size(); j++)
                    {
                        ...
                        if (interior > Math.PI * 2) interior -= Math.PI * 2;
                        
                        if (console) System.out.println(nex.point.x + ", " + nex.point.y + " | " + pos.point.x + ", " + pos.point.y + " | " + test.point.x + ", " + test.point.y + " | " + (interior * 180.0 / Math.PI));
                ...
                if (console) System.out.println("polygon has " + monotone.size() + " points " + (monotone.size() - 2) + " triangles");

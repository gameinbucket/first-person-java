...
public class map extends map_base
{
    ...
    public void bridge(state s, sector add, int add_index, sector base, int base_index)
    {
        ...
        vector[] p =
        {
            add.points[add_index],
            base.points[base_index_plus],
    ...
    public void build()
    {
        ...
        for (int i = 0; i < sector_count; i++)
        {
            ...
            for (sector o : s.inside) for (sector d : o.inside) dead.add(d);
            for (sector d : dead) s.inside.remove(d);
            for (sector o : s.inside) o.outside = s;
        }
    ...
    public void build_lines(sector s)
    {
        ...
        for (int i = 0; i < s.lines.length; i++)
        {
            ...
            float t_s = t_u + (float)Math.sqrt((line.a.x - line.b.x) * (line.a.x - line.b.x) + (line.a.y - line.b.y) * (line.a.y - line.b.y)) * scale;
            
            if (line.bot != null) line.bot.set(bottom, floor, t_u, bottom * scale, t_s, floor * scale);
            if (line.mid != null) line.mid.set(floor, ceil, t_u, floor * scale, t_s, ceil * scale);
            if (line.top != null) line.top.set(ceil, top, t_u, ceil * scale, t_s, top * scale);

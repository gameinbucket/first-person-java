    ...
    public void simulate()
    {
        if (universe.seed.nextInt(100) == 0 && universe.governments.size() > 1)
        {
            for (int j = 0; j < universe.governments.size(); j++)
            {
                ...
                if (wars.contains(g)) continue;
                
                wars.add(g);
                g.wars.add(this);
                
                System.out.println("the " + types[type] + " of " + name + " declared war against the " + types[g.type] + " of " + g.name);
        ...
        else if (wars.size() > 0 && universe.seed.nextInt(100) == 0)
        {
            ...
            System.out.println("the " + types[type] + " of " + name + " declared peace with the " + types[g.type] + " of " + g.name);
        ...
        else if (wars.size() > 0 && universe.seed.nextInt(10) == 0)
        {
            ...
            if (universe.seed.nextInt(3) == 0)
            {
                ...
                System.out.println("the " + types[type] + " of " + name + " defeated the " + types[g.type] + " of " + g.name + " at the " + province.types[p.type] + " " + p.name);
                ...
                if (g.provinces.size() == 0)
                {
                    ...
                    for (int j = 0; j < g.wars.size(); j++) g.wars.get(j).wars.remove(g);
                }
            }
            else
            {
                System.out.println("the " + types[type] + " of " + name + " fought the " + types[g.type] + " of " + g.name + " at the " + province.types[p.type] + " " + p.name);
            }
        }
    }
}

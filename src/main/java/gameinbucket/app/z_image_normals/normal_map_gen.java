...
package gameinbucket.app.z_image_normals;
    ...
    public static void main(String[] arguments) {
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                ...
                /*
                 * double left = depth[x - 1 + y * width]; double right = depth[x + 1 + y *
                 * width]; double top = depth[x + (y - 1) * width]; double bottom = depth[x + (y
                 * + 1) * width];
                 */

                /*
                 * double left = depth[x - 1 + y * width] + sobel[x - 1 + y * width]; double
                 * right = depth[x + 1 + y * width] + sobel[x + 1 + y * width]; double top =
                 * depth[x + (y - 1) * width] + sobel[x + (y - 1) * width]; double bottom =
                 * depth[x + (y + 1) * width] + sobel[x + (y + 1) * width];
                 */
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                /*
                 * int d = (int)Math.floor(depth[i] * 255.0); normal_map.setRGB(x, y, rgb(d, d,
                 * d));
                 */
            }
        }

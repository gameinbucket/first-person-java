...
public class planet {
    ...
    private void tectonics(int plates, int ocean_percent) {
        ...
        while (frontier.isEmpty() == false) {
            ...
            for (hexagon hex : dead)
                frontier.remove(hex);
            for (hexagon hex : found)
                frontier.add(hex);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                for (int j = 0; j < hex.adjacent.length; j++) {
                    ...
                    if (convergent) {
                        if (tectonics[hex.tectonic_id].elevation < tectonics[near.tectonic_id].elevation)
                ...
                if (hex.tectonic_force > intensity)
                    hex.tectonic_force = intensity;
                else if (hex.tectonic_force < -intensity)
                    hex.tectonic_force = -intensity;
            }
        }
    ...
    private void ocean_land() {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                hex.elevation = tectonics[hex.tectonic_id].elevation + hex.tectonic_force + universe.seed.nextInt(11)
                        - 5;
            }
        }
        ...
        for (int blur = 0; blur < 3; blur++) {
            ...
            for (int y = 0; y < height; y++)
                for (int x = 0; x < width; x++)
                    map[x][y].elevation = map[x][y].copy;
        }
    }
    ...
    private void river_lake(int percent) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (hex.elevation >= mountains) {
                    ...
                    for (int j = 0; j < hex.adjacent.length; j++) {
                        ...
                        if (near.elevation > oceans && near.elevation < mountains)
                            nearby.add(near);
                    }
                }
            }
        ...
        if (mountain_count > 0) {
            ...
            while (count * 100 / mountain_count < percent) {
                ...
                while (true) {
                    if (river.elevation <= oceans || river.type == hexagon.river)
                        break;
    ...
    private void winds() {
        ...
        for (int y = 0; y < height; y++)
            for (int x = 0; x < width; x++)
                map[x][y].default_wind = zones[y * zones.length / height];
        ...
        for (int blur = 0; blur < 0; blur++) {
            ...
            for (int y = 0; y < height; y++)
                for (int x = 0; x < width; x++)
                    map[x][y].default_wind = map[x][y].copy;
        }
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (hex.elevation <= oceans || hex.type == hexagon.river) {
                    ...
                    if (hex.temperature < 10)
                        hex.biome = hexagon.tundra;
                    else if (hex.temperature < 40)
                        hex.biome = hexagon.boreal;
                    else if (hex.temperature < 60)
                        hex.biome = hexagon.temperate;
                    else
                        hex.biome = hexagon.tropical;
